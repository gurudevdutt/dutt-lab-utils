...
# Load configuration from canvas_config.json
with open("canvas_config.json", "rt") as f:
    config = json.load(f)

TOKEN = config["token"]
COURSE_ID = config["course_id"]

BASE_URL = f"https://canvas.pitt.edu/api/v1/courses/{COURSE_ID}/"
HEADER = {"Authorization": f"Bearer {TOKEN}"}  # API key


def canvas_client_from_env() -> Dict[str, str]:
    """Build Canvas config from environment variables."""
    ...
    return {"token": token, "course_id": course_id}


def get(url: str, params: Optional[Dict[str, Any]] = None) -> Any:
    r = requests.get(BASE_URL + url, headers=HEADER, json=params)
    r.raise_for_status()
    data = r.json()
...
def post(url: str, params: Dict[str, Any]) -> None:
    r = requests.post(BASE_URL + url, headers=HEADER, json=params)
    r.raise_for_status()


def put(url: str, params: Dict[str, Any]) -> None:
    r = requests.put(BASE_URL + url, headers=HEADER, json=params)
    r.raise_for_status()


def upload(url: str, file: str) -> Dict[str, Any]:
    # step 1
...
def download_attachment(url: str, file_path: str) -> str:
...
def get_assignments() -> Any:
    """Get all assignments in the course."""
    return get("assignments")


def get_assignment(assignment_id: int) -> Any:
    """Get a specific assignment by ID."""
    return get(f"assignments/{assignment_id}")


def get_submissions(assignment_id: int) -> Any:
    """Get all submissions for an assignment."""
    return get(f"assignments/{assignment_id}/submissions")
...
def assign_peer_review(assignment_id: int, submission_id: int, reviewer_id: int) -> Dict[str, Any]:
    """
...
def get_attendance(attempt_lti_endpoint: bool = False) -> Optional[Any]:
    """
    ...
    if attempt_lti_endpoint:
        try:
            ...
            r = requests.get(
                f"https://canvas.pitt.edu/api/v1/courses/{COURSE_ID}/external_tools",
                headers=HEADER,
            )
            r.raise_for_status()
            tools = r.json()
...
def get_student_activity(user_id: Optional[int] = None, limit_assignments: int = 5) -> Dict[str, Any]:
    """
    ...
    if user_id:
        ...
        assignments = get_assignments()[:limit_assignments]  # Limit to avoid too many calls
...
def get_roll_call_attendance() -> Optional[Dict[str, Any]]:
    """
    ...
    try:
        ...
        endpoints_to_try = [
            ...
            f"lti/courses/{COURSE_ID}/attendance",
            f"lti/courses/{COURSE_ID}/roll_call",
        ]

        for endpoint in endpoints_to_try:
            try:
                r = requests.get(
                    f"https://canvas.pitt.edu/api/v1/courses/{COURSE_ID}/{endpoint}",
                    headers=HEADER,
                )
